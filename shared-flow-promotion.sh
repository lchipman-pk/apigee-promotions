...
{
    cat <<!!!!!
    This utility will deploy a shared flow from one Apigee environment and optionally organization to another. It only promotes the most recent revision of the shared flow.

    Syntax:
        shared-flow-promotion.sh [--help|flow] [source_env] [target_env] [source_org] [target_org]
    
    Parameters:
        --help - Returns this help message.
        flow - The name of the shared flow to promote.
...
echo "Apigee Shared Flow Promotion Script v1.0.0"
...
echo "Getting the latest revision of the shared flow from the source..."
#3. Get latest revision in source environment/org
current_revision=$(curl -f -s -H "Authorization: Bearer ${access_token}" ${management_ep_base}organizations/${source_organization}/sharedflows/${api} | jq ".revision | map(tonumber) | max")
if [ -z "${current_revision}" -o "$?" -gt 0 ]
then
    echo "Couldn't determine the current revision of the shared flow (incorrect organization or flow name?)."
    exit 1
fi
...
then
    echo "Source and target organizations differ, exporting and importing shared flow."
    echo "Exporting shared flow from ${source_organization}..."
    export_filename="${api}-rev_${current_revision}.zip"
    curl -f -s -H "Authorization: Bearer ${access_token}" "${management_ep_base}organizations/${source_organization}/sharedflows/${api}/revisions/${current_revision}?format=bundle" -o "${export_filename}"
    if [ -e ${export_filename} -o "$?" -eq 0 ]
    then
        echo "Shared flow exported. You can find a copy at '${export_filename}'."
    else
        echo "Couldn't export the current version of the shared flow (insufficient permissions or out of disk space?)."
        exit 1
    fi

    echo "Importing shared flow to ${target_organization}..."
    #3c. Import revision in target environment/org and get revision ID
    targetorg_import_output=$(curl -f -s -H "Authorization: Bearer ${access_token}" -X POST "${management_ep_base}organizations/${target_organization}/sharedflows?action=import&name=${api}" -F "file=@${export_filename}")
    if [ -z "${targetorg_import_output}" -o "$?" -gt 0 ]
    then
        echo "Couldn't import the new revision of the shared flow (insufficient permissions?)."
        exit 1
    else
        targetorg_revision=$(echo "${targetorg_import_output}" | jq -r ".revision")
        if [ -z "${targetorg_revision}" ]
        then
            echo "Couldn't determine the imported revision of the shared flow."
            exit 1
        fi
    fi
else
...
echo "Deploying shared flow to ${target_env}..."
#4. Deploy latest revision to target environment using seamless
deploy_output=$(curl -f -s -H "Authorization: Bearer ${access_token}" -X POST "${management_ep_base}organizations/${target_organization}/environments/${target_env}/sharedflows/${api}/revisions/${targetorg_revision}/deployments?override=true")
if [ "$?" -gt 0 ]
then
    if [ "${source_organization}" != "${target_organization}" ]
    then
        echo "Deployment failed in seamless mode. The shared flow was imported to \"${target_organization}\" at revision ${targetorg_revision}, but it couldn't deploy to \"${target_env}\" (is the revision already deployed?)."
        exit 1
    else
        echo "Deployment failed in seamless mode. The shared flow couldn't deploy to \"${target_env}\" (is the revision already deployed?)."
        exit 1
    fi
else
